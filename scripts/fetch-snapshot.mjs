...
async function main() {
  ...
  const snapshot = { date: today, total: Math.round(total * 100) / 100, byCategory, byAsset };

  const existingIdx = history.snapshots.findIndex(s => s.date === today);
  if (existingIdx >= 0) history.snapshots[existingIdx] = snapshot; // ya corrió hoy -> actualiza en vez de duplicar
