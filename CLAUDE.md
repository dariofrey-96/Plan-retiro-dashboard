...
- **Los tres archivos generados no se editan a mano**: `css/tokens.css`, `css/presupuesto.css` y los `js/presupuesto*.js` salen de los scripts en `scripts/`. Editar `tokens.css` a mano ya rompió el archivo una vez (una declaración cortada se traga todas las reglas siguientes sin dar error). **Salvedad (desde el 2026-08-31): `js/presupuesto.js` ya se edita directo.** Su generador `traer-presupuesto.mjs` quedó obsoleto — apunta a rutas viejas de OneDrive (`C:/Users/Dario/OneDrive/.../Presupuesto-personal` y el `Plan-retiro-dashboard` de OneDrive) que dejaron de existir cuando se movió el proyecto, y el repo fuente está archivado (todo va acá ahora), así que no se vuelve a correr y no pisa nada. Igual, lo nuevo aditivo se sigue poniendo en archivos aparte no generados (`js/gastos-editar.js`, `js/gastos-params.js`, `css/gastos.css`, `css/cartera.css`) para tocar `presupuesto.js` lo mínimo.
